...
fn main() {
    ...
    let aggregated_signature = json["aggregateSignature"].as_str().expect("Failed to parse value from JSON");
    ...
    stdin.write(&hex::decode(aggregated_signature).expect("Failed to decode aggregated signature from hex"));
