...
pub fn main() {
    ...
    // Call the keccak_n_wasm_wrapper function which is jolt::provable
    let start = std::time::Instant::now();
    let summary = guest::analyze_keccak_n_wasm_wrapper(input, &wasm);
    let duration = start.elapsed();
    println!("Trace length: {:?}", summary.trace_len());
    println!("Time elapsed: {:?}", duration);

    // let (prove_keccak_n_wasm_wrapper, verify_keccak_n_wasm_wrapper) = guest::build_keccak_n_wasm_wrapper();
    // let (output, proof) = prove_keccak_n_wasm_wrapper(input, &wasm);
    // let is_valid = verify_keccak_n_wasm_wrapper(proof);
